...
function registerGeneratorCommand(context: vscode.ExtensionContext): void {
  const build_generate_functions = (opts: GenerateOptions) => {
    return () => {
      ...
      if (filepath) {
        generateAction(filepath, opts).catch((reason) =>
  ...
  const build_github_functions = (_opts: GenerateOptions) => {
    return async () => {
      ...
      if (filepath) {
        ...
        if (token && org && repo) {
          githubPushAction(filepath, token, org, repo).catch((reason: any) =>
            vscode.window.showErrorMessage(reason.message)
          );
        } else {
          vscode.window.showErrorMessage(
            "Configure GITHUB_TOKEN, GITHUB_ORG e GITHUB_REPO no .env do diretório do seu arquivo .made"
          );
        }
      }
    };
    ...
    return async () => {
      ...
      if (filepath) {
        ...
        if (domain && userName && apiToken) {
          ...
          vscode.window.showErrorMessage(
            "Configure JIRA_DOMAIN, JIRA_USERNAME e JIRA_APITOKEN no .env do diretório do seu arquivo .made"
          );
        }
      }
    };
  ...
  const generateDocumentation = build_generate_functions({
    only_project_documentation: true,
  });
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "made.generateDocumentation",
      generateDocumentation
    )
  );

  const generateGithubIssues = build_github_functions({
    only_project_github: true,
  });
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "made.generateGithubIssues",
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("made.githubPush", githubPush)
  );
  ...
  const jiraPush = async () => {
    ...
    if (filepath) {
      ...
      const apiToken = await vscode.window.showInputBox({
        prompt: "Jira APIToken",
      });

      if (domain && userName && apiToken) {
        const { jiraPushAction } = await import("../cli/main.js");
        jiraPushAction(filepath, domain, userName, apiToken).catch(
          (reason: any) => vscode.window.showErrorMessage(reason.message)
        );
      }
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("made.jiraPush", jiraPush)
  );
}
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  try {
    ...
    vscode.window.showErrorMessage(
      `MADE Extension activation failed: ${error}`
    );
  }
}
...
function startLanguageClient(context: vscode.ExtensionContext): LanguageClient {
  ...
  for (const possiblePath of possiblePaths) {
    try {
      ...
      console.log(
        "[MADE Extension] Failed to resolve path:",
        possiblePath,
        error
      );
    }
  ...
  if (!serverModule) {
    const error =
      'Language server not found. Please run "npm run build" to compile the language server.';
